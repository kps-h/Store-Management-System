...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $row['balance'] = $row['total_in_qty'] - $row['total_out_qty'] - $row['total_qty']; // Calculate the balance
        $item_summary[] = $row;
    }
}
...
if (isset($_GET['item_id'])) {
    ...
    $query_item_name = "SELECT Item_Name FROM ledger WHERE Item_Id = '$selected_item_id' LIMIT 1";
    ...
    $query_transactions = "
        SELECT 
            COALESCE(Date_In, Date_Out, date) AS Date,
            Bill_No,
            Voucher_No,
            In_qty,
            Out_qty,
    ...
    if ($result_transactions && $result_transactions->num_rows > 0) {
        ...
        while ($row = $result_transactions->fetch_assoc()) {
            // Calculate balance as In_qty - Out_qty - Qty
            $running_balance += $row['In_qty'];  // Add incoming quantity
            $running_balance -= $row['Out_qty']; // Subtract outgoing quantity
            $running_balance -= $row['Qty'];     // Subtract the additional Qty
    ...
    $(document).ready(function() {
        ...
        <?php if (empty($selected_item_id)): ?>
            $('#itemSummaryTable').DataTable({
                ...
                "columnDefs": [
                    {
                        "targets": [1, 2, 3, 4], // Disable sorting on all columns except the first one
                        "orderable": false
                    }
                ],
        ...
        // DataTable for Transaction Report
        <?php if ($selected_item_id && !empty($transactions)): ?>
            $('#transactionsTable').DataTable({
                ...
                buttons: [
                    ...
                    message: `Item ID: <?php echo htmlspecialchars($selected_item_id); ?> | Item Name: <?php echo htmlspecialchars($item_name); ?>`,
                              
                    customize: function (doc) {
                        // Align the title and message to the center in the PDF
                        doc.styles.title = {
                            fontSize: 18,
                            alignment: 'center'
                        };
                ...
                "columnDefs": [
                    {
                        "targets": [1, 2, 3, 4, 5], // Disable sorting on columns other than "Date"
                        "orderable": false
                    }
                ]
            });
            ...
            <table id="itemSummaryTable" class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <th>In Qty</th>
                        <th>Out Qty</th>
                        <th>Rejected Qty</th> <!-- Added Qty column here -->
                        <th>Balance</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($item_summary)): ?>
                        <?php foreach ($item_summary as $item): ?>
                            <tr>
                                ...
                                <td class="qty-field"><?php echo htmlspecialchars($item['total_qty']); ?></td> <!-- Qty column data -->
        ...
        <?php else: ?>
            <?php if (!empty($transactions)): ?>
                ...
                <table id="transactionsTable" class="table table-bordered">
                    ...
                    <tbody>
                        <?php foreach ($transactions as $transaction): ?>
                            <tr>
                                ...
                                <td class="qty"><?php echo htmlspecialchars($transaction['Qty']); ?></td>
                    ...
                    <tfoot>
                        <tr>
                            <th colspan="6" class="text-right">Final Balance:</th>
